...
class GCN_model(nn.Module):
    def __init__(self, time_steps=3, pretrained=True, partialConv=True, NUM_BOX=12):
        ...
        self.score_head = torch.nn.Linear(128, self.num_bin) # 8 Timestep

        self.risk_type_head = torch.nn.Linear(128, 4)
    ...
    def forward(self, camera_inputs, trackers, mask=None, device='cuda'):
        ...
        for l in range(0, self.time_steps):
            ...
            hx_step = hx.view(batch_size, self.num_box+1, self.hidden_size)  # B×(1+N)×H
