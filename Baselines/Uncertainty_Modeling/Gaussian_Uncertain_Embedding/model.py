...
class GaussianEmbeddingHead(nn.Module):
    """
    let input x through a linear layer to get mu and logvar
    """
    def __init__(self, in_dim: int, emb_dim: int, min_logvar: float = -10.0, max_logvar: float = 2.0):
        super().__init__()
        self.proj = nn.Linear(in_dim, 2 * emb_dim, bias=True)
        self.emb_dim = emb_dim
        self.min_logvar = min_logvar
        self.max_logvar = max_logvar

    def forward(self, x):
        # x: (..., in_dim) -> (..., emb_dim) μ 和 (..., emb_dim) logvar
        h = self.proj(x)
        mu, logvar = torch.split(h, self.emb_dim, dim=-1)
...
class GCN_model(nn.Module):
    def __init__(self, time_steps=3, pretrained=True, partialConv=True, NUM_BOX=12):
        ...
        self.gauss_head = GaussianEmbeddingHead(in_dim=128, emb_dim=128)
        self.score_head = torch.nn.Linear(128, self.num_bin)
    ...
    def forward(self, camera_inputs, trackers, mask=None, intention_inputs=None, state_inputs=None, device='cuda'):
        ...
        mu, logvar = self.gauss_head(x)  
  
        z = GaussianEmbeddingHead.sample(mu, logvar)     # B x N x emb_dim
        ...
        score = self.score_head(z)  # B N num_bin
        score = torch.sigmoid(score)  
        score = score * mask  # B N num_bin
        
        return {'score_H8':score, 'mu':mu, 'logvar':logvar}
