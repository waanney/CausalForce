        ...
        for i in range(B):
            ...
            for j, obj_id in enumerate(all_objs_id):
                if obj_id in gt_risk_ids:
                    var = pred_logvar[j].exp()        # diag(Σ)；larger => more uncertain
                    unc_scalar = var.sum(-1) / 128.0
                    
                    # risk object                                   
                    pred_risk_mask = (pred_risk_score_H8[j] >= (1-unc_scalar.item())).long()         # (8,)
                    ...
                    pred_risk_mask = (pred_risk_score_H8[j] >= (1-unc_scalar)).long()
                    ...
                    self.total_non_risk_obj_cnt += 1
                    pred_risk_mask = (pred_risk_score_H8[j] >= (1-unc_scalar.item())).long()
