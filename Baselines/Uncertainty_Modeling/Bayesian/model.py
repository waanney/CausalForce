...
class GCN_LSTM_BetaBernoulli_model(nn.Module):
    def __init__(self, time_steps=3, pretrained=True, partialConv=True, NUM_BOX=12):
        super(GCN_LSTM_BetaBernoulli_model, self).__init__()
        ...
        self.evidence_head = nn.Linear(128, 2 * self.num_bin)
    ...
    def forward(self, camera_inputs, trackers, mask=None, intention_inputs=None, state_inputs=None, device='cuda'):
        ...
        ev = self.evidence_head(x)                     # shape=(B,N,2*num_bin)
        ev = ev.view(x.size(0), x.size(1), 2, self.num_bin)

        # 2)  softplus to ensure positiveness
        e_pos = F.softplus(ev[:,:,0,:])  # (B,N,num_bin)
        e_neg = F.softplus(ev[:,:,1,:])  # (B,N,num_bin)
        

        # 3) alpha, beta
        alpha = e_pos + 1.0               # (B,N,num_bin)
        beta  = e_neg + 1.0               # (B,N,num_bin)

        # 4) predictive mean & variance
