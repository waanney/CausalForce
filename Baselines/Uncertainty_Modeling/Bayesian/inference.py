        ...
        for i in range(B):
            ...
            for j, obj_id in enumerate(all_objs_id):
                ...
                if obj_id in gt_risk_ids:                               
                    pred_risk_mask = (pred_risk_score_H8[j] >= 0.5)         # (8,)
                    
                    var_mask  = (var_H8[j] / self.var_max) >= self.v_thresh
                    pred_risk_mask = pred_risk_mask | var_mask  # (8,)
                    idx = gt_risk_ids.index(obj_id)
                    gt_risk_mask = gt_risk_score_H8[idx]            # (8,)
                    
                    
                    self.total_gt_risk_obj_cnt += 1 
                    self.total_risk_obj_pred_tube_volume += pred_risk_mask.long().sum().item()
                    self.total_gt_risk_obj_tube_volume += gt_risk_mask.long().sum().item()
                    
                    pred = pred_risk_mask.bool()  # (8,)
                    gt   = gt_risk_mask.bool()  # (8,)
                    ...
                    if torch.all(pred_risk_mask.long()[gt_risk_mask == 1] == 1):
                        self.covered += 1
                    ...
                    pred_risk_mask = (pred_risk_score_H8[j] >= 0.5)         # (8,)
                    var_mask  = (var_H8[j] / self.var_max) >= self.v_thresh
                    pred_risk_mask = pred_risk_mask | var_mask
                    self.total_non_risk_obj_pred_tube_volume += pred_risk_mask.long().sum().item()
...
if __name__ == "__main__":
    ...
    model = GCN_LSTM_BetaBernoulli_model()
