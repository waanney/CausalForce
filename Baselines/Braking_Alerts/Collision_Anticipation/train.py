...
class CA(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(B):
            pred_risk_score = outputs["score"][i]        # (num_preds, 1)
            ...
            if len(all_objs_id) == 0:
                loss_score_i = pred_risk_score.sum() * 0.0

            elif len(gt_risk_ids) == 0:
                gt_risk_score = torch.zeros_like(pred_risk_score[:N])  
                loss_score_i = F.binary_cross_entropy(pred_risk_score[:N], gt_risk_score, reduction='mean')
                ...
                for j, obj_id in enumerate(all_objs_id):
                    if obj_id in gt_risk_ids:
                        matched_pred.append(pred_risk_score[j])          # (1,)
                        idx = gt_risk_ids.index(obj_id)
                        collision = (gt_risk_score[idx] > 0).any().float().unsqueeze(0) # is Collision
                        matched_gt.append(collision)            # (1,)
                    else:
                        non_risk_pred.append(pred_risk_score[j])
                ...
                if len(matched_pred) == 0:
                    ...
                    if len(non_risk_pred) == 0:
                        preds = torch.stack(matched_pred)                      # (P, 1)
                        gts   = torch.stack(matched_gt)                        # (P, 1)
                        loss_score_i = F.binary_cross_entropy(preds, gts, reduction='mean')
                    else:
                        preds = torch.stack(matched_pred)                      # (P, 1)
                        gts   = torch.stack(matched_gt)                        # (P, 1)
            ...
            total_loss += (self.w_score*loss_score_i)
            total_loss_score += self.w_score*loss_score_i
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(B):
            ...
            val_total_loss += (self.w_score*loss_score_i)
            val_total_loss_score += self.w_score*loss_score_i
...
if __name__ == "__main__":
    ...
    parser.add_argument('--id', type=str, default='collision_anticipation', help='Unique experiment identifier.')
    ...
    trainer.fit(CA_Model, dataloader_train, dataloader_val)
