...
class GCN_model(nn.Module):
    ...
    def message_passing(self, input_feature, trackers, device=0):
        ...
        out_attn_weights = attn_weights.clone()
        # (B(1+N))x1
        ...
        return fusion_feature, out_attn_weights[:, 1:, :]
    ...
    def forward(self, camera_inputs, trackers, mask=None, intention_inputs=None, state_inputs=None, device='cuda'):
        ...
        updated_feature, attn_weights = self.message_passing(hx, padded_trackers, device) # B N 512
        
        #x = self.out_layer(self.drop(updated_feature))
       
        isInfluenced = self.vel_classifier(self.drop(updated_feature))
        isInfluenced = self.sigmoid(isInfluenced)

        return {'isInfluenced':isInfluenced, 'attn_weights': attn_weights}
